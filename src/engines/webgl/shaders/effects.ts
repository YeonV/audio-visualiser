...
export const texterShader = `
  ...
  void main() {
    ...
    if (u_textEffect == 0) {
      // Side Scroll
      float scroll = time * 0.2 * (u_speed + 0.1);
      textUV = p + vec2(0.5 + scroll, 0.5);
      textUV.x = fract(textUV.x);
    } else if (u_textEffect == 1 || u_textEffect == 2) {
      // Spokes / Carousel (Simplified: Rotating)
      float rot = time * (u_speed + 0.1);
      float cr = cos(rot), sr = sin(rot);
      vec2 pr = mat2(cr, -sr, sr, cr) * p;
      textUV = pr + 0.5;
    } else if (u_textEffect == 3) {
      // Wave
      float wave = sin(uv.x * 3.0 + time * 4.0) * 0.15 * energy;
      textUV = p + vec2(0.5, 0.5 + wave);
    } else if (u_textEffect == 4) {
      // Pulse
      float pulse = 1.0 + energy * 0.4;
      textUV = (p / pulse) + 0.5;
    } else if (u_textEffect == 5) {
      // Fade
      textUV = p + 0.5;
      alpha = 0.3 + 0.7 * energy;
    } else {
      // Default / Static center
      textUV = p + 0.5;
    }

    // Fix upside down (Canvas vs WebGL)
    ...
    // Glow effect
    finalColor += color * tex.a * alpha * energy * 0.5;
